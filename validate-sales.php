...
if (isset($_POST['search'])) {
	...
	if (!filter_var($_POST["Dollar_Volume"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Dollar Volume";
	}
		if (!filter_var($_POST["Kilo_Volume_Percent_Change"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Kilo Volume Percent Change";
	}
    	if (!filter_var($_POST["Kilo_Vol"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Kilo Vol";
	}
    	if (!filter_var($_POST["Dollar_Rank"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Dollar Rank";
	}
    	if (!filter_var($_POST["Dollar_Volume_PerCentage_Change"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Dollar Volume PerCentage Change";
	}
    	if (!empty($_POST["Average_AC_Dist"])           && !filter_var($_POST["Average_AC_Dist"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Average AC Dist";
	}
	    	if (!filter_var($_POST["Dollar_Share"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Dollar Share";
	}
    	if (!empty($_POST["Average_Retail_Price"])           && !filter_var($_POST["Average_Retail_Price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Average Retail Price";
	}
    	if (!filter_var($_POST["Dollar_Volume_Total"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Dollar Volume Total";
	}
    	if (!filter_var($_POST["Kilo_Volume_Total"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Kilo Volume Total";
	}
    	if (!filter_var($_POST["Kilo_Share"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Kilo Share";
	}
    	if (!empty($_POST["Kilo_Rank"])           && !filter_var($_POST["Kilo_Rank"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
		$sanitation_errors[] = "Kilo Rank";
	}
    	if (!empty($_POST["Product_Grouping"])           && !filter_var($_POST["Product_Grouping"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)) {
